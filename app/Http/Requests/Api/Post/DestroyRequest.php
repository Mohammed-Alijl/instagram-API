    ...
    public function run($id)
    {
        try {
            ...
            foreach ($post->media as $media) {
                $this->isImage($media->media) ? $this->delete_image('img/posts/' . $media->media) : $this->delete_video('video/posts/' . $media->media);
                $media->delete();
            }
            if ($post->delete())
            return $this->apiResponse(null, 200, __('messages.post.destroy'));
